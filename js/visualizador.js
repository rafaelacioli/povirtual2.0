...
const atq1Pel_tte = new Cesium.SingleTileImageryProvider({
...
const atq2Pel_tte = new Cesium.SingleTileImageryProvider({
...
const incamv_tte = new Cesium.SingleTileImageryProvider({
    url: "./media/cpc2023/atq2pel.png",
    rectangle: Cesium.Rectangle.fromDegrees(
        -45.75008,
        -23.49885,
...
const atqamv_lins = new Cesium.SingleTileImageryProvider({
    url: "./media/cpc2023/atqamv_lins.png",
    rectangle: Cesium.Rectangle.fromDegrees(
        -49.998947,
        -22.494886,
        -49.500700,
...
const incamv_lins = new Cesium.SingleTileImageryProvider({
    url: "./media/cpc2023/incamv_lins.png",
    rectangle: Cesium.Rectangle.fromDegrees(
        -49.998947,
        -22.494886,
        -49.500700,
...
// CPC 2023
layers.addImageryProvider(atq1Pel_tte, 11);
layers.addImageryProvider(atq2Pel_tte, 12);
layers.addImageryProvider(incamv_tte, 13);
layers.addImageryProvider(recvig_lins, 14);
layers.addImageryProvider(atqamv_lins, 15);
layers.addImageryProvider(incamv_lins, 16);
...
layers.get(15).show = false;
layers.get(16).show = false;
...
function connectSocket() {
    ...
    let listener = function () {
        ...
        if (coordinationMeasures != null) {
            ...
            removeCoordinationMeasureToCesium(coordinationMeasures)
        }
    }
    ...
    let deleteMeasure = function (id) {
        ...
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 4 && ajax.status >= 200 && ajax.status <= 400) {
                ...
                if (respostaAjax == "ok") {
                    coordinationMeasuresDataSource.entities.removeById(id);
                }
            }
        }
...
function leftClickDefault(e) {
    ...
    if (Cesium.defined(pickedPosition)) {
        ...
        if (pickedPosition.id.isMCC == true) {       // Se a propriedade isMCC estiver definida como TRUE, as opções deverão direcionadas para o DataSource que contém as Medidas de Coordenação 'coordinationMeasuresDataSource'
